...
@ContextConfiguration(
    classes = {TestObjectMapperConfiguration.class, TestWorkflowExecutor.TestConfiguration.class})
...
public class TestWorkflowExecutor {
  ...
  public void testCompleteWorkflow() {
    ...
    verify(workflowStatusListener, times(1)).onWorkflowCompletedIfEnabled(any(WorkflowModel.class));
    verify(workflowStatusListener, times(0)).onWorkflowFinalizedIfEnabled(any(WorkflowModel.class));

    def.setWorkflowStatusListenerEnabled(true);
    workflow.setStatus(WorkflowModel.Status.RUNNING);
    workflowExecutor.completeWorkflow(workflow);
    verify(workflowStatusListener, times(2)).onWorkflowCompletedIfEnabled(any(WorkflowModel.class));
    verify(workflowStatusListener, times(0)).onWorkflowFinalizedIfEnabled(any(WorkflowModel.class));
  }
  ...
  public void testTerminateWorkflow() {
    ...
    verify(workflowStatusListener, times(1)).onWorkflowFinalizedIfEnabled(any(WorkflowModel.class));
  ...
  public void testRetryWorkflowMultipleRetries() {
    ...
    TaskModel lastTask =
        workflow.getTasks().stream()
            ...
            .collect(
                groupingBy(TaskModel::getReferenceTaskName, maxBy(comparingInt(TaskModel::getSeq))))
            .values()
            .stream()
            .map(Optional::get)
  ...
  public void testRetryWorkflowWithJoinTask() {
    ...
    joinTask
        ...
        .put(
            "joinOn",
            Arrays.asList(task_1_1.getReferenceTaskName(), task_2_1.getReferenceTaskName()));
  ...
  public void testGetActiveDomain() throws Exception {
    ...
    PollData pollData1 =
        new PollData("queue1", domains[0], "worker1", System.currentTimeMillis() - 99 * 1000);
    ...
    PollData pollData2 =
        new PollData("queue2", domains[1], "worker2", System.currentTimeMillis() - 99 * 1000);
  ...
  public void testInactiveDomains() {
    ...
    PollData pollData1 =
        new PollData("queue1", domains[0], "worker1", System.currentTimeMillis() - 99 * 10000);
    when(executionDAOFacade.getTaskPollDataByDomain(taskType, domains[0])).thenReturn(pollData1);
  ...
  public void testTaskToDomain() {
    ...
    PollData pollData1 =
        new PollData("queue1", "mydomain", "worker1", System.currentTimeMillis() - 99 * 100);
  ...
  public void testTaskToDomainOverrides() {
    ...
    PollData pollData2 =
        new PollData(
            "queue2", "someActiveDomain", "worker2", System.currentTimeMillis() - 99 * 100);
  ...
  public void testTerminateWorkflowWithFailureWorkflow() {
    ...
    assertEquals(workflow.getWorkflowId(), startWorkflowInput.getWorkflowInput().get("workflowId"));
    assertEquals(
