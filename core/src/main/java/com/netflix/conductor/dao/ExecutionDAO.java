...
public interface ExecutionDAO {
   ...
   *     <p>For a given task reference name and retryCount should be considered unique/primary key.
   *     Given two tasks with the same reference name and retryCount only one should be added to the
   *     database.
   */
  ...
  /**
   * @param workflowId workflow instance id
   * @param includeTasks if set, includes the tasks (pending and completed) sorted by Task Sequence
   *     number in Workflow.
  ...
  /**
   * @return true, if the DAO implementation is capable of searching across workflows false, if the
   *     DAO implementation cannot perform searches across workflows (and needs to use indexDAO)
   */
