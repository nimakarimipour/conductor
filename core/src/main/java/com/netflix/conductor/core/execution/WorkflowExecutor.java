...
public class WorkflowExecutor {
  ...
  private final Predicate<PollData> validateLastPolledTime =
      pollData -> pollData.getLastPollTime() > System.currentTimeMillis() - activeWorkerLastPollMs;
   ...
   * schedule all of them except JOIN (join should change status to INPROGRESS) Switch workflow back
   * to RUNNING status and call decider.
   *
   ...
   * </ul>
   *
   * @param workflowDef The workflow definition of the workflow for which evaluation decision is to
   *     be made
  ...
  private void adjustStateIfSubWorkflowChanged(WorkflowModel workflow) {
    ...
    if (changedSubWorkflowTask.isPresent()) {
      ...
      // Use Optional to safely access the workflow definition
      Optional<WorkflowDef> optionalWorkflowDef =
          Optional.ofNullable(workflow.getWorkflowDefinition());

      // find all terminal and unsuccessful JOIN tasks and set them to IN_PROGRESS
      if (optionalWorkflowDef.map(def -> def.containsType(TaskType.TASK_TYPE_JOIN)).orElse(false)
          || optionalWorkflowDef
              .map(def -> def.containsType(TaskType.TASK_TYPE_FORK_JOIN_DYNAMIC))
              .orElse(false)) {
  ...
  public void pauseWorkflow(String workflowId) {
    try {
      ...
      if (workflow.getStatus().isTerminal()) {
        throw new ConflictException(
            "Workflow %s has ended, status cannot be updated.", workflow.toShortString());
      }
  ...
  public void skipTaskFromWorkflow(
    ...
    // Using the same pattern to ensure WorkflowDefinition is not null
    WorkflowDef workflowDef =
        Optional.ofNullable(workflow.getWorkflowDefinition())
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        String.format(
                            "Workflow definition is undefined for workflow %s", workflowId)));

    // Check if the reference name is as per the workflowdef
    WorkflowTask workflowTask = workflowDef.getTaskByRefName(taskReferenceName);
  ...
  /**
   * Gets the active domain from the list of domains where the task is to be queued. The domain list
   * must be ordered. In sequence, check if any worker has polled for last `activeWorkerLastPollMs`,
   * if so that is the Active domain. When no active domains are found:
  ...
  private WorkflowModel terminate(
    ...
    WorkflowDef workflowDef =
        Optional.ofNullable(workflow.getWorkflowDefinition())
            .orElseThrow(() -> new NullPointerException("Workflow definition is null"));
