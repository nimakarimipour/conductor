...
public class ForkJoinDynamicTaskMapper implements TaskMapper {
  ...
  /**
   * This method gets the list of tasks that need to scheduled when the task to scheduled is of type
   * {@link TaskType#FORK_JOIN_DYNAMIC}. Creates a Fork Task, followed by the Dynamic tasks and a
   * final JOIN task.
   *
   ...
   * WorkflowTask#getDynamicForkJoinTasksParam()} When creating the list of tasks to be scheduled a
   * set of preconditions are validated:
   *
   * <ul>
   *   <li>If the input parameter representing the Dynamic fork tasks is available as part of {@link
   *       WorkflowTask#getDynamicForkTasksParam()} then the input for the dynamic task is validated
   *       to be a map by using {@link WorkflowTask#getDynamicForkTasksInputParamName()}
   *   <li>If the input parameter representing the Dynamic fork tasks is available as part of {@link
   *       WorkflowTask#getDynamicForkJoinTasksParam()} then the input for the dynamic tasks is
   *       available in the payload of the tasks definition.
   *   <li>A check is performed that the next following task in the {@link WorkflowDef} is a {@link
   *       TaskType#JOIN}
   * </ul>
   *
   ...
   *     <ul>
   *       <li>When the task after {@link TaskType#FORK_JOIN_DYNAMIC} is not a {@link TaskType#JOIN}
   ...
   *     and workflowModel
   * @param dynForkTasks: The list of dynamic forked tasks, the reference names of these tasks will
   *     be added to the forkDynamicTask
  ...
  TaskModel createDynamicForkTask(
    ...
    List<String> forkedTaskNames =
        dynForkTasks.stream().map(WorkflowTask::getTaskReferenceName).collect(Collectors.toList());
   ...
   * this#getMappedTasks(TaskMapperContext)} at the end to add a join task to be scheduled after all
   * the fork tasks
   *
   ...
   *
   * @param workflowTask: The Task of type FORK_JOIN_DYNAMIC that needs to scheduled, which has the
   *     input parameters
   ...
   * @throws TerminateWorkflowException : In case of input parameters of the dynamic fork tasks not
   *     represented as {@link Map}
   */
   ...
   * @param workflowModel: The instance of the {@link WorkflowModel} which represents the workflow
   *     being executed.
   * @return {@link Pair} representing the list of dynamic fork tasks in {@link Pair#getLeft()} and
   *     the input for the dynamic fork tasks in {@link Pair#getRight()}
            ...
            .peek(
                dynamicForkJoinTask ->
                    dynamicForkJoinTasksInput.put(
                        ...
                        dynamicForkJoinTask.getInput())) // TODO create a custom pair
            // collector
            .map(
                dynamicForkJoinTask -> {
                  ...
                  if (dynamicForkJoinWorkflowTask.getTaskDefinition() == null
                      && StringUtils.isNotBlank(dynamicForkJoinWorkflowTask.getName())) {
