...
public class SubWorkflow extends WorkflowSystemTask {
  ...
  /**
   * Keep Subworkflow task asyncComplete. The Subworkflow task will be executed once asynchronously
   * to move to IN_PROGRESS state, and will move to termination by Subworkflow's completeWorkflow
   * logic, there by avoiding periodic polling.
   *
   * @param task
   * @return
   */
      ...
      if (!status.isSuccessful()) {
        task.setReasonForIncompletion(
            String.format(
                ...
                subworkflow.toShortString(), subworkflow.getReasonForIncompletion()));
      }
    }
  }
