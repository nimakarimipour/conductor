...
public class DefaultEventQueueProcessor {
    ...
    public DefaultEventQueueProcessor(Map<Status, ObservableQueue> queues, WorkflowExecutor workflowExecutor, ObjectMapper objectMapper) {
        this.queues = queues;
        ...
        queue.observe().subscribe((Message msg) -> {
            try {
                ...
                if (StringUtils.isNotEmpty(taskId)) {
                    optionalTaskModel = workflow.getTasks().stream().filter(task -> !task.getStatus().isTerminal() && task.getTaskId().equals(taskId)).findFirst();
                    ...
                    optionalTaskModel = workflow.getTasks().stream().filter(task -> !task.getStatus().isTerminal() && task.getTaskType().equals(TASK_TYPE_WAIT)).findFirst();
                } else {
                    optionalTaskModel = workflow.getTasks().stream().filter(task -> !task.getStatus().isTerminal() && task.getReferenceTaskName().equals(taskRefName)).findFirst();
                }
