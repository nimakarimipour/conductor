...
public class WorkflowSweeper {
  ...
  public void sweep(String workflowId) {
    ...
    if (workflow != null) {
      ...
      LOGGER.warn(
          "Workflow with {} id can not be found. Attempting to unack using the id", workflowId);
      ...
      if (taskModel.getStatus() == Status.IN_PROGRESS) {
        ...
        if (taskDefinition.isPresent()) {
          ...
          if (taskDef.getPollTimeoutSeconds() != null && taskDef.getPollTimeoutSeconds() != 0) {
            ...
            if (workflowDef != null && workflowDef.getTimeoutSeconds() != 0) {
              postponeDurationSeconds = workflowDef.getTimeoutSeconds() + 1;
            } else {
