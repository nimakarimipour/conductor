...
public class DefaultEventQueueManager extends LifecycleAwareComponent implements EventQueueManager {
  ...
  public void doStart() {
    ...
    defaultQueues.forEach(
        (status, queue) -> {
          LOGGER.info("Start listening on default queue {} for status {}", queue.getName(), status);
          queue.start();
        });
  }
  ...
  public void refreshEventQueues() {
    try {
      ...
      createdQueues.stream().filter(Objects::nonNull).peek(Lifecycle::start).forEach(this::listen);
