...
public class DeciderService {
    ...
    public DeciderService(IDGenerator idGenerator, ParametersUtils parametersUtils, MetadataDAO metadataDAO, ExternalPayloadStorageUtils externalPayloadStorageUtils, SystemTaskRegistry systemTaskRegistry, @Qualifier("taskMappersByTaskType") Map<String, TaskMapper> taskMappers, @Value("${conductor.app.taskPendingTimeThreshold:60m}") Duration taskPendingTimeThreshold) {
    ...
    public DeciderOutcome decide(WorkflowModel workflow) throws TerminateWorkflowException {
        ...
        List<TaskModel> unprocessedTasks = tasks.stream().filter(t -> !t.getStatus().equals(SKIPPED) && !t.isExecuted()).collect(Collectors.toList());
    ...
    @NullUnmarked
    private DeciderOutcome decide(final WorkflowModel workflow, List<TaskModel> preScheduledTasks) throws TerminateWorkflowException {
        ...
        for (TaskModel task : workflow.getTasks()) {
            ...
            if (TERMINATE.name().equals(task.getTaskType()) && task.getStatus().isTerminal() && task.getStatus().isSuccessful()) {
        ...
        for (TaskModel pendingTask : pendingTasks) {
            ...
            if (!pendingTask.getStatus().isSuccessful()) {
                ...
                Optional<TaskModel> retryTask = retry(taskDefinition.orElse(null), workflowTask, pendingTask, workflow);
            ...
            if (!pendingTask.isExecuted() && !pendingTask.isRetried() && pendingTask.getStatus().isTerminal()) {
                ...
                outcome.tasksToBeUpdated.add(pendingTask);
                LOGGER.debug("Scheduling Tasks from {}, next = {} for workflowId: {}", pendingTask.getTaskDefName(), nextTasks.stream().map(TaskModel::getTaskDefName).collect(Collectors.toList()), workflow.getWorkflowId());
            }
        }
        // All the tasks that need to scheduled are added to the outcome, in case of
        List<TaskModel> unScheduledTasks = tasksToBeScheduled.values().stream().filter(task -> !executedTaskRefNames.contains(task.getReferenceTaskName())).collect(Collectors.toList());
        if (!unScheduledTasks.isEmpty()) {
            LOGGER.debug("Scheduling Tasks: {} for workflow: {}", unScheduledTasks.stream().map(TaskModel::getTaskDefName).collect(Collectors.toList()), workflow.getWorkflowId());
            outcome.tasksToBeScheduled.addAll(unScheduledTasks);
        }
        if (hasSuccessfulTerminateTask || (outcome.tasksToBeScheduled.isEmpty() && checkForWorkflowCompletion(workflow))) {
    ...
    @VisibleForTesting
    List<TaskModel> filterNextLoopOverTasks(List<TaskModel> tasks, TaskModel pendingTask, WorkflowModel workflow) {
        ...
        List<String> tasksInWorkflow = workflow.getTasks().stream().filter(runningTask -> runningTask.getStatus().equals(TaskModel.Status.IN_PROGRESS) || runningTask.getStatus().isTerminal()).map(TaskModel::getReferenceTaskName).collect(Collectors.toList());
        return tasks.stream().filter(runningTask -> !tasksInWorkflow.contains(runningTask.getReferenceTaskName())).collect(Collectors.toList());
    }
    ...
    private List<TaskModel> startWorkflow(WorkflowModel workflow) throws TerminateWorkflowException {
        ...
        if (workflow.getReRunFromWorkflowId() == null || tasks.isEmpty()) {
            ...
            WorkflowTask taskToSchedule = // Nothing is running yet - so schedule the first task
            workflowDef.getTasks().// Nothing is running yet - so schedule the first task
            get(0);
        ...
        TaskModel rerunFromTask = tasks.stream().findFirst().map(task -> {
    ...
    void updateWorkflowOutput(final WorkflowModel workflow, TaskModel task) {
        ...
        Optional<TaskModel> optionalTask = allTasks.stream().filter(t -> TaskType.TERMINATE.name().equals(t.getTaskType()) && t.getStatus().isTerminal() && t.getStatus().isSuccessful()).findFirst();
        if (optionalTask.isPresent()) {
            ...
            if (StringUtils.isNotBlank(terminateTask.getExternalOutputPayloadStoragePath())) {
                ...
                Monitors.recordExternalPayloadStorageUsage(terminateTask.getTaskDefName(), Operation.READ.toString(), PayloadType.TASK_OUTPUT.toString());
            ...
            if (workflowDef.getOutputParameters() != null && !workflowDef.getOutputParameters().isEmpty()) {
                output = parametersUtils.getTaskInput(workflowDef.getOutputParameters(), workflow, null, null);
    ...
    public boolean checkForWorkflowCompletion(final WorkflowModel workflow) throws TerminateWorkflowException {
        ...
        boolean noPendingSchedule = nonExecutedTasks.stream().parallel().noneMatch(wftask -> {
    ...
    List<TaskModel> getNextTask(WorkflowModel workflow, TaskModel task) {
        ...
        if (systemTaskRegistry.isSystemTask(task.getTaskType()) && (TaskType.TASK_TYPE_DECISION.equals(task.getTaskType()) || TaskType.TASK_TYPE_SWITCH.equals(task.getTaskType()))) {
    ...
    @VisibleForTesting
    @NullUnmarked
    Optional<TaskModel> retry(TaskDef taskDefinition, WorkflowTask workflowTask, TaskModel task, WorkflowModel workflow) throws TerminateWorkflowException {
        ...
        if (!task.getStatus().isRetriable() || TaskType.isBuiltIn(task.getTaskType()) || expectedRetryCount <= retryCount) {
        ...
        switch(taskDefinition.getRetryLogic()) {
            case FIXED:
                ...
                break;
            case LINEAR_BACKOFF:
                int linearRetryDelaySeconds = taskDefinition.getRetryDelaySeconds() * taskDefinition.getBackoffScaleFactor() * (task.getRetryCount() + 1);
                // Reset integer overflow to max value
                startDelay = linearRetryDelaySeconds < 0 ? Integer.MAX_VALUE : linearRetryDelaySeconds;
                break;
            case EXPONENTIAL_BACKOFF:
                int exponentialRetryDelaySeconds = taskDefinition.getRetryDelaySeconds() * (int) Math.pow(2, task.getRetryCount());
                // Reset integer overflow to max value
                startDelay = exponentialRetryDelaySeconds < 0 ? Integer.MAX_VALUE : exponentialRetryDelaySeconds;
                break;
        }
        task.setRetried(true);
    ...
    void checkWorkflowTimeout(WorkflowModel workflow) {
        ...
        long elapsedTime = workflow.getLastRetriedTime() > 0 ? now - workflow.getLastRetriedTime() : now - workflow.getCreateTime();
        if (elapsedTime < timeout) {
            return;
        }
        String reason = String.format("Workflow timed out after %d seconds. Timeout configured as %d seconds. " + "Timeout policy configured to %s", elapsedTime / 1000L, workflowDef.getTimeoutSeconds(), workflowDef.getTimeoutPolicy().name());
        switch(workflowDef.getTimeoutPolicy()) {
            case ALERT_ONLY:
                LOGGER.info("{} {}", workflow.getWorkflowId(), reason);
                Monitors.recordWorkflowTermination(workflow.getWorkflowName(), WorkflowModel.Status.TIMED_OUT, workflow.getOwnerApp());
                return;
            case TIME_OUT_WF:
    ...
    void checkTaskTimeout(TaskDef taskDef, TaskModel task) {
        ...
        String reason = String.format("Task timed out after %d seconds. Timeout configured as %d seconds. " + "Timeout policy configured to %s", elapsedTime / 1000L, taskDef.getTimeoutSeconds(), taskDef.getTimeoutPolicy().name());
    ...
    void checkTaskPollTimeout(TaskDef taskDef, TaskModel task) {
        ...
        String reason = String.format("Task poll timed out after %d seconds. Poll timeout configured as %d seconds. Timeout policy configured to %s", pollElapsedTime / 1000L, pollTimeout / 1000L, taskDef.getTimeoutPolicy().name());
        timeoutTaskWithTimeoutPolicy(reason, taskDef, task);
    }
    ...
    boolean isResponseTimedOut(TaskDef taskDefinition, TaskModel task) {
        ...
        if (pendingTime > thresholdMS) {
            LOGGER.warn("Task: {} of type: {} in workflow: {}/{} is in pending state for longer than {} ms", task.getTaskId(), task.getTaskType(), task.getWorkflowInstanceId(), task.getWorkflowType(), thresholdMS);
        }
        ...
        if (noResponseTime < adjustedResponseTimeout) {
            LOGGER.debug("Current responseTime: {} has not exceeded the configured responseTimeout of {} for the Task: {} with Task Definition: {}", pendingTime, responseTimeout, task, taskDefinition);
            return false;
        }
    ...
    @NullUnmarked
    public List<TaskModel> getTasksToBeScheduled(WorkflowModel workflow, WorkflowTask taskToSchedule, int retryCount) {
        return getTasksToBeScheduled(workflow, taskToSchedule, retryCount, null);
    }

    @NullUnmarked
    public List<TaskModel> getTasksToBeScheduled(WorkflowModel workflow, WorkflowTask taskToSchedule, int retryCount, String retriedTaskId) {
        ...
        TaskMapperContext taskMapperContext = TaskMapperContext.newBuilder().withWorkflowModel(workflow).withTaskDefinition(taskToSchedule.getTaskDefinition()).withWorkflowTask(taskToSchedule).withTaskInput(input).withRetryCount(retryCount).withRetryTaskId(retriedTaskId).withTaskId(taskId).withDeciderService(this).build();
        ...
        return taskMappers.getOrDefault(type, taskMappers.get(USER_DEFINED.name())).getMappedTasks(taskMapperContext).stream().filter(task -> !tasksInWorkflow.contains(task.getReferenceTaskName())).collect(Collectors.toList());
    }
    ...
    private boolean isAyncCompleteSystemTask(TaskModel task) {
        return systemTaskRegistry.isSystemTask(task.getTaskType()) && systemTaskRegistry.get(task.getTaskType()).isAsyncComplete(task);
    }
