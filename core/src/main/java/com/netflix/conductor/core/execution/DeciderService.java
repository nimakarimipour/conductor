...
public class DeciderService {
    ...
    private List<TaskModel> startWorkflow(WorkflowModel workflow) throws TerminateWorkflowException {
        ...
        if (workflow.getReRunFromWorkflowId() == null || tasks.isEmpty()) {
            ...
            WorkflowTask // Nothing is running yet - so schedule the first task
            // Nothing is running yet - so schedule the first task
            taskToSchedule = workflowDef.getTasks().get(0);
    ...
    @VisibleForTesting
    Optional<TaskModel> retry(@Nullable TaskDef taskDefinition, WorkflowTask workflowTask, TaskModel task, WorkflowModel workflow) throws TerminateWorkflowException {
    ...
    public List<TaskModel> getTasksToBeScheduled(WorkflowModel workflow, WorkflowTask taskToSchedule, int retryCount, @Nullable String retriedTaskId) {
