...
public class DeciderService {
      ...
      if (taskDefinition.isEmpty()) {
        taskDefinition =
            Optional.ofNullable(workflow.getWorkflowDefinition())
                .map(
                    workflowDef -> workflowDef.getTaskByRefName(pendingTask.getReferenceTaskName()))
        ...
        if (workflowTask == null) {
          workflowTask =
              Optional.ofNullable(workflow.getWorkflowDefinition())
                  .map(
                      workflowDef ->
                          workflowDef.getTaskByRefName(pendingTask.getReferenceTaskName()))
                  .orElse(null);
        }
  ...
  private List<TaskModel> startWorkflow(WorkflowModel workflow) throws TerminateWorkflowException {
    ...
    if (workflow.getReRunFromWorkflowId() == null || tasks.isEmpty()) {
      ...
      WorkflowTask taskToSchedule =
          workflowDef.getTasks().get(0); // Nothing is running yet - so schedule the first task
   ...
   * @param task if not null, the output of this task will be copied to workflow output if no output
   *     parameters are specified in the workflow definition if null, the output of the last task in
   *     the workflow will be copied to workflow output of no output parameters are specified in the
   *     workflow definition
   */
  void updateWorkflowOutput(final WorkflowModel workflow, @Nullable TaskModel task) {
    ...
    if (optionalTask.isPresent()) {
      ...
      if (workflowDef.getOutputParameters() != null
        ...
        output =
            parametersUtils.getTaskInput(workflowDef.getOutputParameters(), workflow, null, null);
      } else if (StringUtils.isNotBlank(last.getExternalOutputPayloadStoragePath())) {
        output =
            externalPayloadStorageUtils.downloadPayload(last.getExternalOutputPayloadStoragePath());
        Monitors.recordExternalPayloadStorageUsage(
            last.getTaskDefName(), Operation.READ.toString(), PayloadType.TASK_OUTPUT.toString());
      } else {
    ...
    switch (workflowDef.getTimeoutPolicy()) {
      case ALERT_ONLY:
        ...
        Monitors.recordWorkflowTermination(
            workflow.getWorkflowName(), WorkflowModel.Status.TIMED_OUT, workflow.getOwnerApp());
        return;
      case TIME_OUT_WF:
