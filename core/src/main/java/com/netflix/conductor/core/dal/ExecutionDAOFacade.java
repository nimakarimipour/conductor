...
public class ExecutionDAOFacade {
  ...
  public ExecutionDAOFacade(
    ...
    this.scheduledThreadPoolExecutor =
        new ScheduledThreadPoolExecutor(
            ...
            (runnable, executor) -> {
              LOGGER.warn(
                  "Request {} to delay updating index dropped in executor {}", runnable, executor);
  ...
  public void shutdownExecutorService() {
    try {
      ...
      if (scheduledThreadPoolExecutor.awaitTermination(
        ...
        LOGGER.warn(
            "Forcing shutdown after waiting for {} seconds", properties.getAsyncUpdateDelay());
  ...
  private WorkflowModel getWorkflowModelFromDataStore(
      @Nullable String workflowId, boolean includeTasks) {
  ...
  /**
   * Retrieve all workflow executions with the given correlationId and workflow type Uses the {@link
   * IndexDAO} to search across workflows if the {@link ExecutionDAO} cannot perform searches across
   * workflows.
   *
    ...
    if (properties.isAsyncIndexingEnabled()) {
      if (workflowModel.getStatus().isTerminal()
        ...
        scheduledThreadPoolExecutor.schedule(
            delayWorkflowUpdate, properties.getAsyncUpdateDelay().getSeconds(), TimeUnit.SECONDS);
      ...
      if (workflowModel.getStatus().isTerminal()) {
        workflowModel
            .getTasks()
            .forEach(taskModel -> indexDAO.asyncIndexTask(new TaskSummary(taskModel.toTask())));
      }
    } else {
      indexDAO.indexWorkflow(new WorkflowSummary(workflowModel.toWorkflow()));
    }
  ...
  /**
   * Sets the update time for the task. Sets the end time for the task (if task is in terminal state
   * and end time is not set). Updates the task in the {@link ExecutionDAO} first, then stores it in
   * the {@link IndexDAO}.
   *
  ...
  /**
   * Save the {@link EventExecution} to the data store Saves to {@link ExecutionDAO} first, if this
   * succeeds then saves to the {@link IndexDAO}.
   *
  ...
  /**
   * Populates the workflow input data and the tasks input/output data if stored in external payload
   * storage.
   *
