...
public class SimpleActionProcessor implements ActionProcessor {
    ...
    public SimpleActionProcessor(WorkflowExecutor workflowExecutor, ParametersUtils parametersUtils, JsonUtils jsonUtils, StartWorkflowOperation startWorkflowOperation) {
    ...
    public Map<String, Object> execute(Action action, @Nullable Object payloadObject, String event, String messageId) {
        ...
        switch(action.getAction()) {
            ...
            case complete_task:
                return completeTask(action, jsonObject, action.getComplete_task(), TaskModel.Status.COMPLETED, event, messageId);
            case fail_task:
                return completeTask(action, jsonObject, action.getFail_task(), TaskModel.Status.FAILED, event, messageId);
            default:
                break;
        }
    ...
    private Map<String, Object> completeTask(Action action, @Nullable Object payload, TaskDetails taskDetails, TaskModel.Status status, String event, String messageId) {
    ...
    private Map<String, Object> startWorkflow(Action action, @Nullable Object payload, String event, String messageId) {
        ...
        try {
            ...
            Optional.ofNullable(params.getCorrelationId()).ifPresent(value -> paramsMap.put("correlationId", value));
            ...
            startWorkflowInput.setCorrelationId(Optional.ofNullable(replaced.get("correlationId")).map(Object::toString).orElse(params.getCorrelationId()));
