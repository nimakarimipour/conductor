...
public class Monitors {
   ...
   * a simple queue, counters would be used to measure things like the rate at which items are being
  ...
  /**
   * Set a gauge is a handle to get the current value. Typical examples for gauges would be the size
   * of a queue or number of threads in the running state. Since gauges are sampled, there is no
   * information about what might have occurred between samples.
   *
   * @param className
   * @param name
   * @param measurement
  ...
  public static void recordTaskPollError(
      String taskType, @Nullable String domain, String exception) {
    counter(
        classQualifier,
        "task_poll_error",
        "taskType",
