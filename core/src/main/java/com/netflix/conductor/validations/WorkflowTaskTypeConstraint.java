...
public @interface WorkflowTaskTypeConstraint {
  ...
  class WorkflowTaskValidator
    ...
    private boolean isWaitTaskValid(WorkflowTask workflowTask, ConstraintValidatorContext context) {
      boolean valid = true;
      String duration =
      ...
      if (StringUtils.isNotBlank(duration) && StringUtils.isNotBlank(until)) {
        String message = "Both 'duration' and 'until' specified. Please provide only one input";
        context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
        valid = false;
      }
    ...
    private boolean isTerminateTaskValid(
      ...
      if (workflowTask.isOptional()) {
        String message =
            String.format(
                "terminate task cannot be optional, taskName: %s", workflowTask.getName());
        context.buildConstraintViolationWithTemplate(message).addConstraintViolation();
        valid = false;
      }
