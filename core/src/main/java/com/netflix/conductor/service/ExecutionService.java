...
public class ExecutionService {
    ...
    public List<Task> poll(String taskType, String workerId, @Nullable String domain, int count, int timeoutInMilliSecond) {
    ...
    public Task getPendingTaskForWorkflow(String taskReferenceName, String workflowId) {
        return // There can only be one task by a given reference name running at a
        // time.
        executionDAOFacade.getTasksForWorkflow(workflowId).stream().filter(task -> !task.getStatus().isTerminal()).filter(task -> task.getReferenceTaskName().equals(taskReferenceName)).findFirst().orElse(null);
    }
