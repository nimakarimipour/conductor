...
public class ExecutionService {
  ...
  public Task getLastPollTask(String taskType, String workerId, String domain) {
    ...
    if (tasks.isEmpty()) {
      LOGGER.debug(
          "No Task available for the poll: /tasks/poll/{}?{}&{}", taskType, workerId, domain);
      return null;
    }
    Task task = tasks.get(0);
    ackTaskReceived(task);
    LOGGER.debug(
        "The Task {} being returned for /tasks/poll/{}?{}&{}", task, taskType, workerId, domain);
    return task;
  }
              ...
              queueName -> {
                try {
                  if (!queueName.contains(QueueUtils.DOMAIN_SEPARATOR)) {
                    allPollData.addAll(
                        getPollData(QueueUtils.getQueueNameWithoutDomain(queueName)));
                  }
                } catch (Exception e) {
        ...
        .filter(
            workflow -> {
              if (includeClosed || workflow.getStatus().equals(Workflow.WorkflowStatus.RUNNING)) {
                ...
                if (includeTasks) {
                  List<Task> tasks =
                      executionDAOFacade.getTasksForWorkflow(workflow.getWorkflowId());
            ...
            .map(
                taskSummary -> {
                  try {
                    ...
                    LOGGER.error(
                        "Error fetching workflow by id: {}", taskSummary.getWorkflowId(), e);
                    return null;
                  }
                })
            .filter(Objects::nonNull)
  ...
  public ExternalStorageLocation getExternalStorageLocation(
    ...
    try {
      ...
      String errorMsg =
          String.format("Invalid input - Operation: %s, PayloadType: %s", operation, type);
