...
public class WorkflowBulkServiceImpl implements WorkflowBulkService {
   ...
   * @return bulk response object containing a list of succeeded workflows and a list of failed ones
   *     with errors
   */
  ...
  public BulkResponse restart(List<String> workflowIds, boolean useLatestDefinitions) {
    ...
    for (String workflowId : workflowIds) {
      try {
        ...
        LOGGER.error(
            "bulk restart exception, workflowId {}, message: {} ", workflowId, e.getMessage(), e);
        bulkResponse.appendFailedResponse(workflowId, e.getMessage());
      }
    }
    return bulkResponse;
  ...
  public BulkResponse retry(List<String> workflowIds) {
    ...
    for (String workflowId : workflowIds) {
      try {
        ...
        LOGGER.error(
            "bulk retry exception, workflowId {}, message: {} ", workflowId, e.getMessage(), e);
        bulkResponse.appendFailedResponse(workflowId, e.getMessage());
      }
    }
    return bulkResponse;
  ...
  public BulkResponse terminate(List<String> workflowIds, String reason) {
    ...
    for (String workflowId : workflowIds) {
      try {
        ...
        LOGGER.error(
            "bulk terminate exception, workflowId {}, message: {} ", workflowId, e.getMessage(), e);
        bulkResponse.appendFailedResponse(workflowId, e.getMessage());
      }
    }
    return bulkResponse;
